...
import indoorData from '../../public/indoor-data.json'
import {
  findNearestNode,
  isInsideCampus,
  startGPSTracking,
...
const TYPE_CONFIG = {
  gate:     { color: '#ff4d6d', glow: 'rgba(255,77,109,0.4)',   bg: 'rgba(255,77,109,0.12)',   icon: DoorOpen,        label: 'Gate' },
  admin:    { color: '#fbbf24', glow: 'rgba(251,191,36,0.4)',   bg: 'rgba(251,191,36,0.12)',   icon: Landmark,        label: 'Admin' },
  academic: { color: '#60a5fa', glow: 'rgba(96,165,250,0.4)',   bg: 'rgba(96,165,250,0.12)',   icon: GraduationCap,   label: 'Academic' },
  parking:  { color: '#94a3b8', glow: 'rgba(148,163,184,0.4)', bg: 'rgba(148,163,184,0.12)', icon: ParkingSquare,   label: 'Parking' },
  amenity:  { color: '#34d399', glow: 'rgba(52,211,153,0.4)',   bg: 'rgba(52,211,153,0.12)',   icon: UtensilsCrossed, label: 'Amenity' },
  landmark: { color: '#c084fc', glow: 'rgba(192,132,252,0.4)', bg: 'rgba(192,132,252,0.12)', icon: MapPin,          label: 'Landmark' },
  sports:   { color: '#22d3ee', glow: 'rgba(34,211,238,0.4)',   bg: 'rgba(34,211,238,0.12)',   icon: Trophy,          label: 'Sports' },
}
...
function getRoomType(roomName) {
  ...
  if (n.includes('washroom') || n.includes('toilet')) return 'washroom'
  if (n.includes('faculty') || n.includes('staff') || n.includes('cabin') || n.includes('dean') || n.includes('hod')) return 'faculty'
  if (n.includes('dept') || n.includes('department') || n.includes('office') || n.includes('meeting')) return 'department'
  if (n.includes('lab') || n.includes('laboratory')) return 'lab'
  if (n.includes('research')) return 'research'
  if (n.includes('hall') || n.includes('auditorium') || n.includes('tutorial')) return 'hall'
  if (n.includes('lecture') || n.includes('class') || n.includes('room') || n.includes('tutorial')) return 'class'
  if (n.includes('empty') || n.includes('not available')) return 'empty'
  return 'other'
}

// Build direction text based on room number
function getDirections(blockKey, floorKey, roomId, roomName) {
  const blockData = indoorData[blockKey]
  if (!blockData) return []
  const floorNum = parseInt(floorKey)
  const blockNum = blockKey === 'block3' ? 3 : 2
  const floorLabel = blockData.floors[floorKey]?.name || `Floor ${floorNum}`

  const dirs = []
  dirs.push(`Head to Academic Block ${blockNum} main entrance.`)
  if (floorNum === 0) {
    dirs.push('The room is on the Ground Floor — no stairs needed.')
  } else {
    dirs.push(`Take the staircase up to the ${floorLabel}.`)
    if (floorNum >= 5) dirs.push('Alternatively, use the lift if available.')
  }
  // Left/right hint based on room number suffix
  const suffix = parseInt(roomId.slice(-2))
  if (suffix <= 6) {
    dirs.push('Turn left at the top of the stairs.')
  } else {
    dirs.push('Turn right at the top of the stairs.')
  }
  dirs.push(`Look for room ${roomId} — ${roomName}.`)
  return dirs
}
...
function createGPSMarkerElement() {
  ...
  el.innerHTML = `
    <div id="gps-arrow" style="
      ...
      border-bottom:14px solid #38bdf8;filter:drop-shadow(0 0 4px rgba(56,189,248,0.8));
    "></div>
    <div style="position:absolute;width:24px;height:24px;top:4px;left:4px;
      background:rgba(56,189,248,0.25);border-radius:50%;animation:gps-pulse 2s ease-out infinite;"></div>
    <div style="position:absolute;width:40px;height:40px;top:-4px;left:-4px;
      background:rgba(56,189,248,0.1);border-radius:50%;animation:gps-pulse 2s ease-out infinite 0.5s;"></div>
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
      width:14px;height:14px;background:#38bdf8;border:2.5px solid #fff;border-radius:50%;
      box-shadow:0 0 10px rgba(56,189,248,0.9);z-index:2;"></div>
  `
  return el
}
...
function IndoorPanel({ onClose }) {
  ...
  return (
    <div className="indoor-panel">
      ...
      {/* Search */}
      <div className="indoor-search-wrap">
...
// ─── Faculty Locator Panel ──────────────────────────────────────────────────
function FacultyPanel({ onClose }) {
  const [query, setQuery] = useState('')

  // Collect all faculty from indoor data
  const allFaculty = []
  Object.entries(indoorData).forEach(([blockKey, blockData]) => {
    Object.entries(blockData.floors).forEach(([floorKey, floorData]) => {
      Object.entries(floorData.rooms).forEach(([roomId, roomName]) => {
        if (getRoomType(roomName) === 'faculty') {
          allFaculty.push({
            blockLabel: blockData.name,
  ...
  const filtered = query.trim()
    ? allFaculty.filter(f =>
        f.roomName.toLowerCase().includes(query.toLowerCase()) ||
        f.roomId.includes(query) ||
        f.floorLabel.toLowerCase().includes(query.toLowerCase())
      )
    : allFaculty
  ...
  return (
    <div className="indoor-panel">
      <div className="indoor-panel-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Users size={16} color="#fbbf24" strokeWidth={2.2} />
          <span className="indoor-panel-title" style={{ color: '#fbbf24' }}>Faculty Locator</span>
        </div>
        <button className="sjec-close" style={{ position: 'static' }} onClick={onClose}>
          <X size={16} strokeWidth={2.2} />
        </button>
      ...
      <div className="indoor-search-wrap">
        ...
        <input
          className="indoor-search"
          placeholder="Search faculty cabin or floor..."
          value={query}
          onChange={e => setQuery(e.target.value)}
          autoFocus
        />
      ...
      <div className="faculty-list">
        {filtered.length === 0 && (
          <div className="indoor-empty-hint">
            <Users size={28} strokeWidth={1.5} color="#fbbf24" style={{ opacity: 0.4 }} />
            <p>No faculty cabins found.</p>
          </div>
        )}
        {filtered.map((f, i) => (
          <div key={i} className="faculty-item">
            <div className="faculty-item-icon">
              <Users size={14} color="#fbbf24" strokeWidth={2.2} />
            </div>
            <div className="faculty-item-info">
              <div className="faculty-item-name">{f.roomName}</div>
              <div className="faculty-item-meta">
                Room {f.roomId} · {f.floorLabel} · {f.blockLabel}
              </div>
            </div>
          </div>
        ))}
...
function BuildingPopup({ blockKey, onClose }) {
  ...
  const [activeFloor, setActiveFloor] = useState(
    Object.keys(blockData.floors).sort((a, b) => parseInt(a) - parseInt(b))[0]
  )

  const floorKeys = Object.keys(blockData.floors).sort((a, b) => parseInt(a) - parseInt(b))

  return (
    <div className="building-popup">
      <div className="building-popup-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Building2 size={15} color="#60a5fa" strokeWidth={2.2} />
          <span className="building-popup-title">{blockData.name}</span>
        </div>
        <button className="sjec-close" style={{ position: 'static' }} onClick={onClose}>
      ...
      {/* Floor tabs */}
      <div className="building-floor-tabs">
        {floorKeys.map(fk => (
          <button
            key={fk}
      ...
      <div className="building-rooms-list">
        {Object.entries(blockData.floors[activeFloor]?.rooms || {}).map(([rid, rname]) => (
          <div key={rid} className="building-room-row">
            <span className="building-room-dot" style={{ background: ROOM_TYPE_COLOR[getRoomType(rname)] }} />
            <span className="building-room-id">{rid}</span>
            <span className="building-room-name">{rname}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function MapView({ userRole = 'visitor', roleLabel = 'Visitor' }) {
  ...
  const [buildingPopup, setBuildingPopup] = useState(null) // blockKey string
  ...
  const addAllLayers = useCallback((map) => {
    ...
    if (!map.getSource('outside-mask')) map.addSource('outside-mask', { type: 'geojson', data: OUTSIDE_MASK })
    if (!map.getLayer('outside-mask-layer')) {
      map.addLayer({ id: 'outside-mask-layer', type: 'fill', source: 'outside-mask',
        paint: { 'fill-color': '#020812', 'fill-opacity': 0.88 } })
    }

    if (!map.getSource('campus-boundary')) map.addSource('campus-boundary', { type: 'geojson', data: CAMPUS_BOUNDARY })
    ...
    if (!map.getSource('edges')) map.addSource('edges', { type: 'geojson', data: buildEdgesGeoJSON() })
    ...
    if (!map.getLayer('gps-accuracy-circle')) {
      map.addLayer({ id: 'gps-accuracy-circle', type: 'circle', source: 'gps-accuracy',
        paint: {
    ...
    if (!map.getSource('nodes')) map.addSource('nodes', { type: 'geojson', data: buildNodesGeoJSON() })

    if (!map.getLayer('nodes-glow')) {
      map.addLayer({ id: 'nodes-glow', type: 'circle', source: 'nodes',
        paint: {
          'circle-radius': ['interpolate', ['linear'], ['zoom'], 14, 2, 16, 5, 18, 10, 20, 14],
          'circle-color': ['match', ['get', 'type'],
            'gate', '#ff4d6d', 'admin', '#fbbf24', 'academic', '#60a5fa',
            'parking', '#94a3b8', 'amenity', '#34d399', 'landmark', '#c084fc',
            'sports', '#22d3ee', '#60a5fa'],
    ...
    if (!map.getLayer('nodes-bg')) {
      map.addLayer({ id: 'nodes-bg', type: 'circle', source: 'nodes',
        paint: {
          'circle-radius': ['interpolate', ['linear'], ['zoom'], 14, 1.5, 16, 3, 17, 5, 18, 7, 19, 9, 20, 11],
          'circle-color': ['match', ['get', 'type'],
            'gate', 'rgba(255,77,109,0.1)', 'admin', 'rgba(251,191,36,0.1)',
            'academic', 'rgba(96,165,250,0.1)', 'parking', 'rgba(148,163,184,0.1)',
            'amenity', 'rgba(52,211,153,0.1)', 'landmark', 'rgba(192,132,252,0.1)',
            'sports', 'rgba(34,211,238,0.1)', 'rgba(96,165,250,0.1)'],
    ...
    if (!map.getLayer('nodes-label')) {
      map.addLayer({ id: 'nodes-label', type: 'symbol', source: 'nodes', minzoom: 17,
        layout: {
          'text-field': ['get', 'label'],
          'text-font': ['DIN Offc Pro Medium', 'Arial Unicode MS Bold'],
          'text-size': ['interpolate', ['linear'], ['zoom'], 17, 8, 18, 10, 20, 12],
          'text-offset': [0, 1.2], 'text-anchor': 'top', 'text-max-width': 8, 'text-allow-overlap': false,
        },
        paint: {
          'text-color': '#cbd5e1', 'text-halo-color': '#020812', 'text-halo-width': 2,
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({
      ...
      center: SJEC_CENTER, zoom: 16.8,
      minZoom: 15.5, maxZoom: 20,
      maxBounds: SJEC_BOUNDS,
      pitch: 52, bearing: -15, antialias: true,
    })
    mapRef.current = map
    map.on('dragstart', () => { followUserRef.current = false })
    map.addControl(new mapboxgl.NavigationControl({ visualizePitch: true }), 'bottom-right')
    map.on('load', () => addAllLayers(map))
    map.on('style.load', () => { styleHandlersBoundRef.current = false; addAllLayers(map) })
    return () => { map.remove(); mapRef.current = null; styleHandlersBoundRef.current = false }
  }, [addAllLayers])
  ...
  const handleLocationUpdate = useCallback((location) => {
    ...
    const inside = isInsideCampus(location.lat, location.lng, CAMPUS_BOUNDARY.features[0].geometry.coordinates)
    ...
    if (userMarkerRef.current) {
      ...
      userMarkerRef.current = new mapboxgl.Marker({ element: el, anchor: 'center', rotationAlignment: 'map' })
    ...
    if (map.getSource('gps-accuracy')) {
      map.getSource('gps-accuracy').setData({
        type: 'FeatureCollection',
        features: [{ type: 'Feature', geometry: { type: 'Point', coordinates: [location.lng, location.lat] } }]
      })
    }
    if (followUserRef.current) {
      const opts = { center: [location.lng, location.lat], duration: 1000, essential: true }
      if (location.heading !== null && location.heading !== undefined) {
        opts.bearing = location.heading; opts.pitch = 60; opts.zoom = 18.2
      }
      map.easeTo(opts)
    }
    if (inside && location.accuracy <= 20) {
      const { node } = findNearestNode(location.lat, location.lng, campusData.nodes)
      if (node) { setNearestNode(node); setRouteFrom(node.id) }
    } else { setNearestNode(null) }
  }, [])

  const toggleLiveTracking = useCallback(() => {
    if (liveTracking) {
      stopGPSTracking(watchIdRef.current); watchIdRef.current = null
      setLiveTracking(false); setGpsStatus(''); setNearestNode(null); setGpsAccuracy(null)
      followUserRef.current = true
      if (userMarkerRef.current) { userMarkerRef.current.remove(); userMarkerRef.current = null }
      mapRef.current?.getSource('gps-accuracy')?.setData({ type: 'FeatureCollection', features: [] })
    } else {
      followUserRef.current = true
      const id = startGPSTracking(handleLocationUpdate, (err) => { console.error(err); setGpsStatus('error') })
      watchIdRef.current = id; setLiveTracking(true)
    }
  ...
  const recenterOnUser = useCallback(() => {
    ...
    if (userMarkerRef.current) {
      ...
      mapRef.current?.easeTo({ center: [lngLat.lng, lngLat.lat], zoom: 18.2, pitch: 60, bearing: currentRotation, duration: 800 })
    }
  }, [])

  useEffect(() => { return () => { stopGPSTracking(watchIdRef.current) } }, [])

  const drawRoute = useCallback((path) => {
    const map = mapRef.current
    if (!map?.getSource('route')) return
    const coords = path.map(id => { const n = campusData.nodes.find(x => x.id === id); return [n.lng, n.lat] })
    ...
    map.fitBounds(bounds, { padding: 100, pitch: isFlat ? 0 : 52, bearing: isFlat ? 0 : -15, duration: 1400 })
  }, [viewMode])
  ...
  const flyToNode = (node) => {
    const isFlat = viewMode === '2d' || viewMode === 'satellite'
    mapRef.current?.flyTo({ center: [node.lng, node.lat], zoom: 18.5, pitch: isFlat ? 0 : 55, bearing: isFlat ? 0 : -15, duration: 1000 })
  ...
  const cycleViewMode = () => {
    ...
    if (next === '3d')        map.easeTo({ pitch: 52,  bearing: -15, duration: 700 })
    else if (next === '2d')   map.easeTo({ pitch: 0,   bearing: 0,   duration: 700 })
    else if (next === 'satellite') { map.setStyle('mapbox://styles/mapbox/satellite-streets-v12'); map.easeTo({ pitch: 0, bearing: 0, duration: 700 }) }
    else if (next === 'night')     { map.setStyle('mapbox://styles/mapbox/navigation-night-v1');  map.easeTo({ pitch: 52, bearing: -15, duration: 700 }) }
  }

  const resetView = () => {
    const isFlat = viewMode === '2d' || viewMode === 'satellite'
    mapRef.current?.flyTo({ center: SJEC_CENTER, zoom: 16.8, pitch: isFlat ? 0 : 52, bearing: isFlat ? 0 : -15, duration: 1200 })
  }
  ...
  return (
    <div className="sjec-root">
      ...
      <div className="sjec-header">
        <div className="sjec-logo"><GraduationCap size={20} strokeWidth={2.2} /></div>
        ...
        <div className="sjec-search-wrap">
          <span className="sjec-search-icon"><Search size={16} strokeWidth={2.2} /></span>
          <input className="sjec-search" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search any place on campus..." />
      ...
      <div className="sjec-controls">
        <button className="sjec-btn" onClick={cycleViewMode}>
          <span>{viewMode === '3d' ? <Building2 size={16} strokeWidth={2.1} /> : viewMode === '2d' ? <Map size={16} strokeWidth={2.1} /> : viewMode === 'satellite' ? <Satellite size={16} strokeWidth={2.1} /> : <Moon size={16} strokeWidth={2.1} />}</span>
          <span>{viewMode === '3d' ? '3D' : viewMode === '2d' ? '2D' : viewMode === 'satellite' ? 'Sat' : 'Night'}</span>
        </button>

        <button className="sjec-btn" onClick={resetView}>
          <span><Crosshair size={16} strokeWidth={2.1} /></span><span>Reset</span>
        </button>

        <button className={`sjec-btn ${routeMode ? 'active danger' : ''}`} onClick={() => { setRouteMode(!routeMode); setSelectedNode(null); if (routeMode) clearRoute() }}>
          <span>{routeMode ? <X size={16} strokeWidth={2.1} /> : <Compass size={16} strokeWidth={2.1} />}</span>
          <span>{routeMode ? 'Cancel' : 'Route'}</span>
        </button>

        <button className={`sjec-btn ${liveTracking ? 'active' : ''}`} onClick={toggleLiveTracking}>
          <span>{liveTracking ? <Radio size={16} strokeWidth={2.1} /> : <LocateFixed size={16} strokeWidth={2.1} />}</span>
          <span>{liveTracking ? 'Live' : 'Track Me'}</span>
        </button>

        {liveTracking && (
          <button className="sjec-btn" onClick={recenterOnUser}>
            <span><Crosshair size={16} strokeWidth={2.1} /></span><span>Recenter</span>
          </button>
        )}

        {/* Indoor Navigation button */}
        <button
          className={`sjec-btn ${showIndoor ? 'active indoor-active' : 'indoor-btn'}`}
          onClick={() => { setShowIndoor(!showIndoor); setShowFaculty(false) }}
        >
          <span><Navigation size={16} strokeWidth={2.1} /></span>
          <span>Indoor Nav</span>
        </button>

        {/* Faculty Locator button */}
        <button
          className={`sjec-btn ${showFaculty ? 'active faculty-active' : 'faculty-btn'}`}
          onClick={() => { setShowFaculty(!showFaculty); setShowIndoor(false) }}
        >
          <span><Users size={16} strokeWidth={2.1} /></span>
          <span>Faculty</span>
        </button>
      </div>
      ...
      {liveTracking && (
        <div style={{
          ...
          background: gpsStatus === 'inside' ? 'rgba(52,211,153,0.12)' : gpsStatus === 'outside' ? 'rgba(255,77,109,0.12)' : 'rgba(56,189,248,0.12)',
          border: `1px solid ${gpsStatus === 'inside' ? '#34d399' : gpsStatus === 'outside' ? '#ff4d6d' : '#38bdf8'}`,
          borderRadius: 20, padding: '5px 16px', fontSize: 11,
          color: gpsStatus === 'inside' ? '#34d399' : gpsStatus === 'outside' ? '#ff4d6d' : '#38bdf8',
          backdropFilter: 'blur(8px)', zIndex: 15, display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap',
        }}>
          <span style={{ display: 'flex', alignItems: 'center' }}>
            {gpsStatus === 'inside' ? <ShieldCheck size={14} strokeWidth={2.2} /> : gpsStatus === 'outside' ? <TriangleAlert size={14} strokeWidth={2.2} /> : <Search size={14} strokeWidth={2.2} />}
          </span>
          {gpsStatus === 'inside' ? `You are indoors${gpsAccuracy ? ` · ±${gpsAccuracy}m` : ''}` : gpsStatus === 'outside' ? 'You are outside campus' : 'Getting GPS signal...'}
        </div>
      )}

      {/* Indoor Navigation Panel — replaces legend position */}
      {showIndoor && (
        <IndoorPanel onClose={() => setShowIndoor(false)} />
      )}

      {/* Faculty Locator Panel */}
      {showFaculty && (
        <FacultyPanel onClose={() => setShowFaculty(false)} />
      )}

      {/* Building double-click popup */}
      {buildingPopup && (
        <BuildingPopup blockKey={buildingPopup} onClose={() => setBuildingPopup(null)} />
      )}

      {/* Node Info Panel */}
      {selectedNode && !routeMode && (
        <div className="sjec-panel">
          <button className="sjec-close" onClick={() => setSelectedNode(null)}><X size={16} strokeWidth={2.2} /></button>
          <div className="sjec-info-header">
            <div className="sjec-info-icon" style={{ background: TYPE_CONFIG[selectedNode.type]?.bg, border: `1px solid ${TYPE_CONFIG[selectedNode.type]?.color}30`, boxShadow: `0 0 16px ${TYPE_CONFIG[selectedNode.type]?.glow}` }}>
              {TYPE_CONFIG[selectedNode.type]?.icon && (() => { const Icon = TYPE_CONFIG[selectedNode.type].icon; return <Icon size={18} strokeWidth={2.1} color={TYPE_CONFIG[selectedNode.type]?.color} /> })()}
            </div>
            <div>
              <div className="sjec-info-name">{selectedNode.label}</div>
              <div className="sjec-info-type" style={{ color: TYPE_CONFIG[selectedNode.type]?.color }}>{TYPE_CONFIG[selectedNode.type]?.label}</div>
            </div>
          </div>
          <div className="sjec-info-coords" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <button className="sjec-nav-btn" onClick={() => openRouteFrom(selectedNode)}>
            <Compass size={16} strokeWidth={2.1} /> Navigate from here
          </button>
        </div>
      )}
      ...
      {routeMode && (
        <div className="sjec-panel">
          <div className="sjec-route-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Compass size={16} strokeWidth={2.1} /> Route Planner
          </div>
          {liveTracking && nearestNode && (
            <div style={{ fontSize: 11, color: '#34d399', background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 8, padding: '5px 10px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
              <Radio size={14} strokeWidth={2.2} /> Auto-detected: {nearestNode.label}
            </div>
          )}
          ...
          {routeResult && (
            <div className="sjec-route-result">
              <div className="sjec-route-stats">
                <div className="sjec-stat"><div className="sjec-stat-val">{formatDistance(routeResult.distance)}</div><div className="sjec-stat-label">Distance</div></div>
                <div className="sjec-stat"><div className="sjec-stat-val">{routeResult.path.length - 1}</div><div className="sjec-stat-label">Stops</div></div>
                <div className="sjec-stat"><div className="sjec-stat-val">{getWalkingETA(routeResult.distance)}</div><div className="sjec-stat-label">Walk time</div></div>
              </div>
          ...
          {routeFrom && routeTo && !routeResult && (
            <div className="sjec-no-route" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <TriangleAlert size={15} strokeWidth={2.2} /> No path found between these locations
            </div>
          )}
      ...
      {!mapLoaded && (
        <div className="sjec-loading">
          <div className="sjec-loading-logo"><LoaderCircle size={24} strokeWidth={2.2} className="spin" /></div>
